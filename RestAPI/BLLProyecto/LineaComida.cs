...
namespace BLLProyecto
{
    public class LineaComida
    {
        ...
        {
            conn = DAL.traerConexion("Progra5", ref mensajeError, ref numError);
            if (conn == null)
            {
            ...
            else
            {
                ...
                DAL.agregarEstructuraParametros(ref parametros, 0, "@cod_planta", SqlDbType.Int, codigoLineaComida);
                DAL.conectar(conn, ref mensajeError, ref numError);
                DAL.ejecutarSqlCommandParametros(conn, sql, true, parametros, ref mensajeError, ref numError);
                if (numError != 0)
                {
